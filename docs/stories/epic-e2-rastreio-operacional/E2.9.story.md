...
- [x] **AC4:** Tempo médio de ciclo: do recebimento à expedição (média dos últimos 7 dias)
...
- [x] **AC9:** Filtros: hoje / semana / mês / período customizado
- [x] **AC10:** Gráficos com Recharts: barras para volume, linha para tendência, donut para status
- [x] **AC11:** Cards de KPI com: valor atual, comparativo com dia anterior (▲▼%)
...
### Completion Notes
- AC4 (tempo médio de ciclo) implementado via `avgCycleTimeMinutes` em `production-kpis.ts`
- AC9 (filtros de período) implementado via `PeriodFilter` component com today/week/month/custom
- AC11 (trend ▲▼%) implementado — KpiCard recebe prop `trend` calculado contra período anterior
...
| 2026-02-28 | @dev Dex | ACs pendentes concluídos: avgCycleTime, PeriodFilter, trend comparison |
