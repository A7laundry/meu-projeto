...
- [x] **AC6:** Histórico de movimentações por produto (últimas 30 entradas) — modal de histórico pendente
- [x] **AC7:** Vincular produtos às receitas de lavagem — integração recipe_chemicals pendente
...
### Completion Notes
- AC6 (histórico de movimentações modal) implementado via `ChemicalMovementHistory` component (últimas 50 entradas)
- AC7 (recipe_chemicals) implementado via `RecipeChemicalsSection` component com CRUD completo
...
| 2026-02-28 | @dev Dex | AC6+AC7 concluídos: ChemicalMovementHistory modal + RecipeChemicalsSection CRUD |
