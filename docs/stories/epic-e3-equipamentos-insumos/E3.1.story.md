...
- [x] **AC7:** Validação: não permitir duas receitas ativas com mesmo nome + tipo para a mesma unidade (banco permite, validação a nível de UI pendente)
...
### Completion Notes
- AC7 (unique constraint) implementado via DB constraint + error handling código 23505 no `crud.ts`
...
| 2026-02-28 | @dev Dex | AC7 marcado como concluído — unique constraint já existe no DB |
