...
import { getNetworkPayables } from '@/actions/director/network-payables'
import { NetworkPayableTable } from '@/components/domain/director/network-payable-table'
...
export default async function PayablesPage() {
  ...
  const { payables, totals } = await getNetworkPayables()
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white">Contas a Pagar da Rede</h1>
        <p className="text-sm text-white/40 mt-1">
          Gestão multi-unidade de contas a pagar
        </p>
      </div>
      ...
      <NetworkPayableTable payables={payables} units={unitList} />
    </div>
  )
}
