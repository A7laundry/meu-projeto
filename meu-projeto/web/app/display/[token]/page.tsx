...
import { TvRefresher } from '@/app/tv/[unitId]/refresher'
...
export default async function DisplayTokenPage({ params }: Props) {
  ...
  const unitId = tokenData.unit_id
    ...
    <div className="flex flex-col" style={{ minHeight: '100vh', padding: '20px 24px', gap: 20 }}>
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-6">
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 32, fontWeight: 800, color: '#fff', lineHeight: 1 } as React.CSSProperties}>
              {orders.length}
            </p>
      ...
      <div className="flex gap-4 flex-1" style={{ overflow: 'hidden' }}>
        {ACTIVE_STAGES.map((stage) => {
          ...
          return (
            <div key={stage} className="flex flex-col flex-1" style={{ gap: 10, minWidth: 0 }}>
              <div
                style={{
                ...
                {items.length === 0 ? (
                  ...
                  return (
                    <div
                      key={order.id}
                      style={{
                        background: isReady ? 'rgba(59,130,246,0.07)' : 'rgba(255,255,255,0.03)',
                        border: isReady ? '1px solid rgba(59,130,246,0.20)' : '1px solid rgba(255,255,255,0.07)',
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                        <p
                          style={{
                            fontSize: isReady ? 18 : 16, fontWeight: 700,
                            color: isReady ? '#60a5fa' : 'rgba(255,255,255,0.90)', lineHeight: 1.2,
                          }}
                        >
                          #{order.order_number}
                        </p>
